...
class Object_Follow(_cam_based_class):
    ...
    def __draw_bboxes(self, img, boxes, clss, idx, mx):
        for i, (bb, cl) in enumerate(zip(boxes, clss)):
            cl = int(cl)
            ...
            txt = '{}'.format(self.cls_dict.get(cl, 'CLS{}'.format(cl)))
    ...
    def detect(self, image=None, index=None, threshold=0.5, show=True, callback=None):
        ...
        boxes, classes = self.__postprocess_yolo(self.trt_outputs, width, height, threshold, nms_threshold=0.5, input_shape=self.input_shape)
        ...
        for bb, cl in zip(boxes, classes):
            detections.append({'label' : int(cl), 
                            ...
                            'x' : round(((bb[0]+bb[2])/2)/(width/2)-1, 2), 
                            'y' : round(((bb[1]+bb[3])/2)/(height/2)-1, 2), 
                            'size_rate': round((bb[2]-bb[0])*(bb[3]-bb[1])/(width * height), 2)})
        ...
        if show:
            image = self.__draw_bboxes(image, boxes, classes, index, max_size)
            if isnotebook:
...
###############################################################################################################################
        ...
        # input1 = keras.layers.Input(shape=(150, 300, 3,))
        # conv1 = keras.layers.Conv2D(filters=16, kernel_size=(3, 3), strides=(2, 2), padding="same")(input1)
        # norm1 = keras.layers.BatchNormalization()(conv1)
        # act1 = keras.layers.Activation(mish)(norm1)
        # pool1 = keras.layers.MaxPooling2D(pool_size=(3, 3), strides=(2, 2))(act1)
        # conv2 = keras.layers.Conv2D(filters=32, kernel_size=(3, 3), strides=(2, 2), padding="same")(pool1)
        # norm2 = keras.layers.BatchNormalization()(conv2)
        # act2 = keras.layers.Activation(mish)(norm2)
        # conv3 = keras.layers.Conv2D(filters=32, kernel_size=(3, 3), strides=(1, 1), padding="same")(act2)
        # norm3 = keras.layers.BatchNormalization()(conv3)
        # act3 = keras.layers.Activation(mish)(norm3)
        # add1 = keras.layers.Add()([act2, act3])
        # conv4 = keras.layers.Conv2D(filters=64, kernel_size=(3, 3), strides=(2, 2), padding="same")(add1)
        # norm4 = keras.layers.BatchNormalization()(conv4)
        # act4 = keras.layers.Activation(mish)(norm4)
        # conv5 = keras.layers.Conv2D(filters=64, kernel_size=(3, 3), strides=(1, 1), padding="same")(act4)
        # norm5 = keras.layers.BatchNormalization()(conv5)
        # act5 = keras.layers.Activation(mish)(norm5)
        # add2 = keras.layers.Add()([act4, act5])
        # conv6 = keras.layers.Conv2D(filters=128, kernel_size=(3, 3), strides=(2, 2), padding="same")(add2)
        # norm6 = keras.layers.BatchNormalization()(conv6)
        # act6 = keras.layers.Activation(mish)(norm6)
        # conv7 = keras.layers.Conv2D(filters=128, kernel_size=(3, 3), strides=(1, 1), padding="same")(act6)
        # norm7 = keras.layers.BatchNormalization()(conv7)
        # act7 = keras.layers.Activation(mish)(norm7)
        # add3 = keras.layers.Add()([act6, act7])
        # conv8 = keras.layers.Conv2D(filters=256, kernel_size=(3, 3), strides=(2, 2), padding="same")(add3)
        # norm8 = keras.layers.BatchNormalization()(conv8)
        # act8 = keras.layers.Activation(mish)(norm8)
        # conv9 = keras.layers.Conv2D(filters=512, kernel_size=(3, 3), strides=(2, 2), padding="same")(act8)
        # norm9 = keras.layers.BatchNormalization()(conv9)
        # act9 = keras.layers.Activation(mish)(norm9)
        # flat1 = keras.layers.Flatten()(act9)
        # dense1 = keras.layers.Dense(128)(flat1)
        # norm10 = keras.layers.BatchNormalization()(dense1)
        # act10 = keras.layers.Activation(softplus)(norm10)
        # dense2 = keras.layers.Dense(64)(act10)
        # norm11 = keras.layers.BatchNormalization()(dense2)
        # act11 = keras.layers.Activation(softplus)(norm11)
        # dense3 = keras.layers.Dense(64)(act11)
        # norm12 = keras.layers.BatchNormalization()(dense3)
        # act12 = keras.layers.Activation(softplus)(norm12)
        # dense4 = keras.layers.Dense(2)(act12)
    ...
    def load_model(self,path=MODEL_PATH):
        ...
        self.model=load_model(path, custom_objects={'mish': mish}, compile=False)
        ...
        #self.model.load_weights(path, custom_objects={'mish': mish}, compile=False)
    ...
    def run(self, value=None, callback=None):
        ...
        self.value = cv2.circle(img, (cX, 205), 6, (255, 255, 255), 2)
    ...
    def run_TF(self, value=None, callback=None):
        ...
        img = self.camera.value if value is None else value
        crop_img = img[130:280,:300]
        x, y, z = self.model(np.array([crop_img]).astype(np.float32)).numpy()[0]
        height, width,_ = crop_img.shape
        cX = int(width * (x))
        cY = int(height * (y))
        ...
        if isnotebook:
            ...
            result={"x":x,"y":y,"z":z,"cx":cX,"cy":cY}
